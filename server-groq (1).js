...
async function extractRequest(text) {
  ...
  if (!aiKey) return basicExtract(text);
  const usingGroq = Boolean(GROQ_API_KEY);
  const response = await fetch(usingGroq ? "https://api.groq.com/openai/v1/chat/completions" : "https://api.openai.com/v1/chat/completions", {
    method: "POST",
    headers: { Authorization: `Bearer ${aiKey}`, "Content-Type": "application/json" },
    body: JSON.stringify({
      model: usingGroq ? GROQ_TEXT_MODEL : OPENAI_TEXT_MODEL,
      temperature: 0,
...
async function transcribeAudio(mediaId) {
  const transcriptionKey = GROQ_API_KEY || OPENAI_API_KEY;
  if (!transcriptionKey) throw new Error("Falta GROQ_API_KEY para transcribir notas de voz");
  const usingGroq = Boolean(GROQ_API_KEY);
  ...
  form.append("model", usingGroq ? GROQ_TRANSCRIBE_MODEL : OPENAI_TRANSCRIBE_MODEL);
  form.append("language", "es");
  const response = await fetch(usingGroq ? "https://api.groq.com/openai/v1/audio/transcriptions" : "https://api.openai.com/v1/audio/transcriptions", {
    method: "POST", headers: { Authorization: `Bearer ${transcriptionKey}` }, body: form
  });
  if (!response.ok) {
    let detail = "";
    try {
      const errorBody = await response.json();
      detail = String(errorBody?.error?.message || "").slice(0, 180);
    } catch {}
    throw new Error(`No se pudo transcribir el audio (${response.status})${detail ? `: ${detail}` : ""}`);
  }
  const data = await response.json();
