...
function money(value) {
  ...
  return Number.isFinite(number)
    ? new Intl.NumberFormat("es-NI", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(number)
    : "0.00";
}

async function notifyAssignedOrder(data) {
  const phone = String(data.telefono || data.phone || "").replace(/\D/g, "");
  const rider = String(data.motorizado || data.rider || "nuestro motorizado").trim();
  const service = Number(data.costoServicio ?? data.deliveryCost ?? 0);
  ...
  await sendWhatsAppMessage(
    ...
    [
      ...
      `Motorizado asignado: ${rider}`,
      `Costo del mandado: C$${money(service)}`,
      "",
      "Mandados Chevita ya está gestionando tu pedido."
    ].join("\n")
  );
}
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === "POST" && url.pathname === "/appsheet/order-assigned") {
    if (!APPSHEET_WEBHOOK_SECRET || req.headers["x-chevita-secret"] !== APPSHEET_WEBHOOK_SECRET) {
      return send(res, 403, "No autorizado");
    }

    readJsonBody(req)
      .then(async (data) => {
        await notifyAssignedOrder(data);
        console.log(`Cliente notificado por asignación: ${data.telefono || data.phone || "desconocido"}`);
        send(res, 200, JSON.stringify({ ok: true }), "application/json; charset=utf-8");
      })
      .catch((error) => {
        console.error("No se pudo notificar la asignación:", error.message);
